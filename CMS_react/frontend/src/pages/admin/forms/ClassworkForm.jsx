...
const ClassworkForm = ({ isEditing, classworkId }) => {
  ...
  useEffect(() => {
    fetchTrainings();
    fetchQuizzes();
    if (isEditing && classworkId) {
      fetchClassworkData();
    }
  }, [isEditing, classworkId]);
  ...
  const fetchClassworkData = async () => {
    try {
      setLoading(true);
      const res = await apiClient.get(`/training/classwork/${classworkId}/`);
      ...
      setFormData({
        ...
        linked_quiz: res.linked_quiz || ''
      });
      
      if (res.classwork_file) {
        setCurrentFileUrl(res.classwork_file);
        setContentMode('pdf');
      } else if (res.linked_quiz) {
        setContentMode('quiz');
      } else {
  ...
  const handleFileChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      setClassworkFile(e.target.files[0]);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (contentMode === 'quiz' && formData.linked_quiz) {
      submitData.append('linked_quiz', formData.linked_quiz);
    }

    if (contentMode === 'pdf' && classworkFile) {
      submitData.append('classwork_file', classworkFile);
    }

    try {
      if (isEditing) {
        await apiClient.put(`/training/classwork/${classworkId}/`, submitData);
      } else {
        await apiClient.post('/training/classwork/', submitData);
      }
      navigate(-1);
    } catch (err) {
      console.error(err);
      setError(err.message || 'Failed to save classwork.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h2 className="text-2xl font-bold text-slate-800">
          {isEditing ? 'Edit Classwork' : 'Create Classwork Assignment'}
        </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
                <label className="block text-sm font-bold text-slate-700 mb-2">Classwork Title *</label>
                <input
                  type="text"
                  name="title"
                  ...
                  placeholder="e.g. Mid-term Data Analysis Project"
                />
              </div>
              ...
              <div className="md:col-span-2 border-t border-slate-200 pt-6 mt-2">
                ...
                <div className="flex gap-4 mb-6">
                  ...
                  >
                    ...
                    <span className="font-semibold">Link Quiz</span>
                    <span className="text-xs mt-1 text-center">Use existing course quiz</span>
                  </button>
                </div>

                {contentMode === 'editor' && (
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                    <label className="block text-sm font-bold text-slate-700 mb-2">Write Assignment Content</label>
                    <Editor
                ...
                {contentMode === 'pdf' && (
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                    <label className="block text-sm font-bold text-slate-700 mb-2">Upload Classwork PDF</label>
                    <input
                      type="file"
                      accept=".pdf,.doc,.docx"
                ...
                {contentMode === 'quiz' && (
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                    <label className="block text-sm font-bold text-slate-700 mb-2">Select Existing Quiz/Exam</label>
                    <select
                      name="linked_quiz"
                      value={formData.linked_quiz}
                      onChange={handleChange}
                      className="w-full p-3 border border-slate-300 rounded-lg bg-white"
                    >
            ...
            <div className="flex justify-end pt-6 border-t border-slate-200">
              <Button type="submit" disabled={saving} className="bg-[#0A66C2] hover:bg-blue-700 text-white min-w-[120px]">
                {saving ? 'Saving...' : (isEditing ? 'Save Changes' : 'Create Classwork')}
              </Button>
            </div>
          </form>
        </CardContent>
