...
const SuperAdminLayout = () => {
  ...
  const courseManagementItems = [
    { path: '/superadmin', label: 'Dashboard', icon: <LayoutDashboard size={18} /> },
        ...
        <div className="h-20 flex items-center justify-between px-6 shrink-0 pt-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[8px] text-slate-400 tracking-widest">LEARN • GROW • SUCCEED</p>
            </div>
          </div>
        ...
        <header className="h-[90px] bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0 z-10">
          ...
          <div className="flex items-center gap-6">
            <button className="relative p-2 text-slate-700 hover:bg-slate-100 rounded-full transition-colors">
              <Bell size={22} />
              <span className="absolute top-1.5 right-1.5 w-[18px] h-[18px] bg-[#0A66C2] text-white text-[10px] font-bold flex items-center justify-center rounded-full border-2 border-white">3</span>
            </button>
            <div className="hidden md:flex items-center gap-3 pl-6 border-l border-slate-200 cursor-pointer hover:opacity-80">
              <div className="w-11 h-11 rounded-full bg-slate-200 overflow-hidden border border-slate-200">
                <img src="https://i.pravatar.cc/150?u=superadmin" alt="Profile" className="w-full h-full object-cover" />
              </div>
              <div className="text-sm">
                <p className="font-bold text-slate-900 text-[14px] leading-tight">{user?.username || 'Superadmin'}</p>
